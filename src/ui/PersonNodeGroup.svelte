<script>
	...
	for (let i = 0; i < personNodeGroupData.groupMembers.length; i++) {
		if (personNodeGroupData.groupMembers[i] && typeof personNodeGroupData.groupMembers[i] === 'string' && personNodeGroupData.groupMembers[i].constructor === String) {

			personNodeGroupData.groupMembers[i] = findPersonByName(personNodeGroupData.groupMembers[i], true)
